...
describe('evaluateGuardrails', () => {
  it('reports exposure and daily loss as fractions of equity', () => {
    const g = evaluateGuardrails({ equity: 1000, openStake: 20, openCount: 1, dailyRealizedPnl: -15, limits });
  ...
  it('flags a breach and auto-disable when the daily loss limit is hit', () => {
    const g = evaluateGuardrails({ equity: 1000, openStake: 0, openCount: 0, dailyRealizedPnl: -30, limits });
  ...
  it('flags exposure breach but not auto-disable (exposure alone does not disable)', () => {
    const g = evaluateGuardrails({ equity: 1000, openStake: 45, openCount: 2, dailyRealizedPnl: 0, limits });
  ...
  it('flags the open-position cap', () => {
    const g = evaluateGuardrails({ equity: 1000, openStake: 10, openCount: 3, dailyRealizedPnl: 0, limits });
    expect(g.openCountBreached).toBe(true);
    expect(g.anyBreached).toBe(true);
  });

  it('ignores daily profit (only losses count toward the limit)', () => {
    const g = evaluateGuardrails({ equity: 1000, openStake: 0, openCount: 0, dailyRealizedPnl: 500, limits });
    expect(g.dailyLossPct).toBe(0);
    expect(g.dailyLossBreached).toBe(false);
  });

  it('treats zero equity safely (no divide-by-zero)', () => {
    const g = evaluateGuardrails({ equity: 0, openStake: 10, openCount: 1, dailyRealizedPnl: -5, limits });
