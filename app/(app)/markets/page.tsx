...
import { titleCase, formatSymbolName } from '@/lib/utils/format';
          ...
          <Link key={e.instrumentSymbol} href={`/signals/${e.instrumentSymbol}`}>
            <Card className="transition-colors hover:border-accent/50">
              <div className="flex items-center justify-between">
                <span className="font-semibold text-fg">{formatSymbolName(e.instrumentSymbol)}</span>
                <BiasBadge bias={e.htf.bias} />
              </div>
              <div className="mt-1 text-xs text-muted">{e.instrumentSymbol} · {titleCase(e.family)}</div>
